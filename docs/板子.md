...
```c++
vector<Point> polygon_from_lines(const vector<Line> &lines) {
	vector<Point> polygon;
	polygon.reserve(lines.size());
	for(int i = 0; i < (int)lines.size(); ++i)
		polygon.emplace_back(*inter(lines[i], lines[(i + 1) % lines.size()]));
	return polygon;
}
...
ld polygon_circ(const vector<Point> &polygon) {
	ld ret = 0.0l;
	for(int i = 0; i < (int)polygon.size(); ++i)
		ret += dist(polygon[i], polygon[(i + 1) % polygon.size()]);
	return ret;
}
```
...
```c++
ld inter_area(const vector<Point> &polygon, const Circle &c) {
...
```c++
array<int, 3> convex_max_triangle(const vector<Point> &convex) {
	int m = convex.size();
	if(m < 3) return {-1, -1, -1};
	auto point = [&](int i) -> const Point & { return convex[i % m]; };
	ld max_area = -1.0l;
	...
	for(int i = 0; i < m; ++i) {
		...
		for(int j = i + 1; j < i + m - 1; ++j) {
			while(k + 1 < i + m &&
				  cross(point(j) - point(i), point(k + 1) - point(i)) >
					  cross(point(j) - point(i), point(k) - point(i)))
				++k;
			ld area = 0.5l * cross(point(j) - point(i), point(k) - point(i));
...
pair<int, int> convex_farthest(const vector<Point> &convex) {
	int m = convex.size();
	if(m == 0) return {-1, -1};
	if(m == 1) return {0, 0};
	if(m == 2) return {0, 1};
	...
	for(int i = 0; i < m; ++i) {
		int u = (i + 1) % m;
		while(cross(convex[i], convex[u], convex[(j + 1) % m]) >
			  cross(convex[i], convex[u], convex[j]))
			j = (j + 1) % m;
		auto chmax = [&](int k) {
			ld now = (convex[k] - convex[j]).len2();
...
bool convex_is_in(const Point &pt, const vector<Point> &convex) {
	if(convex.empty()) return false;
	if(convex.size() == 1) return pt == convex[0];
	if(convex.size() == 2) return is_on(pt, LineSeg(convex[0], convex[1]));
	...
	int n = convex.size();
	Point pivot = convex[0];
	if(sign(cross(convex[1] - pivot, pt - pivot)) == -1 ||
	   sign(cross(convex[n - 1] - pivot, pt - pivot)) == 1)
		return false;
	int l = 1, r = n - 1;
	while(l + 1 < r) {
		int mid = (l + r) >> 1;
		if(sign(cross(convex[mid] - pivot, pt - pivot)) != -1) l = mid;
		else r = mid;
	}
	int nxt = (l == n - 1) ? 1 : l + 1;
	return check(pivot, convex[l], convex[nxt], pt);
}

opt<pair<Point, Point>> convex_tangent(const Point &pt,
									   const vector<Point> &convex) {
	if(convex_is_in(pt, convex)) return nul;
	int n = convex.size();
	auto peak = [&](int l, int r, bool f) {
		while(l < r - 1) {
			int m = (l + r) / 2;
			if(f ^ (to_left(convex[(m + n - 1) % n] - pt, convex[m] - pt) == 1))
				r = m;
			else l = m;
		}
		return l;
	};
	if(to_left(convex[0] - pt, convex[1] - pt) == 0) {
		int idx =
			peak(2, n, cmp(dist(convex[0], pt), dist(convex[1], pt)) == -1);
		return pair{convex[0], convex[idx]};
	}
	...
	chk(to_left(convex[0] - pt, convex[1] - pt));
	chk(to_left(convex[0] - pt, convex[n - 1] - pt));
	if(al || ar) {
		int id = peak(1, n, al);
		return pair{convex[0], convex[id]};
	}
	int l = 1, r = n;
	while(l < r - 1) {
		int mid = (l + r) / 2;
		if(to_left(convex[0] - pt, convex[mid] - pt) == 1) l = mid;
		else r = mid;
	}
	bool f = to_left(convex[0] - pt, convex[1] - pt) == 1;
	int i1 = peak(0, l + 1, f), i2 = peak(l, n, !f);
	return pair{convex[i1], convex[i2]};
}

pair<Point, Point> convex_tangent(const Line &ln, const vector<Point> &convex) {
	int n = convex.size();
	...
	auto find = [&](bool f) {
		...
		while(r - l > 2) {
			...
			ld dot1 = dot(convex[m1], dir), dot2 = dot(convex[m2], dir);
			if(f ^ (cmp(dot1, dot2) == -1)) r = m2;
			else l = m1;
		}
		int idx = l;
		ld ret = dot(convex[l], dir);
		for(int i = l; i <= r; ++i) {
			ld now = dot(convex[i], dir);
			if(f ^ (cmp(now, ret) == -1)) {
				idx = i;
				ret = now;
			}
	...
	return {convex[i1], convex[i2]};
}

vector<Point> convex_minkowski_add(const vector<Point> &a,
...
vector<Point> convex_minkowski_sub(const vector<Point> &a,
								   const vector<Point> &b) {
	vector<Point> neg_b;
	neg_b.reserve(b.size());
	for(const auto &pt : b) neg_b.push_back(Point() - pt);
	return convex_minkowski_add(a, neg_b);
}
```
