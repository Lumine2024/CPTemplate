...
```c++
template<ll modulo> constexpr ll norm(ll x) {
	x %= modulo;
	if(x < 0) x += modulo;
	return x;
}
...
```c++
template<ll modulo> struct Lucas {
	Lucas() = delete;
	static ll binom(ll n, ll m) {
		ensure_init();
		if(m < 0 || m > n) return 0;
...
```c++
template<ll modulo, ll primitive_root> void ntt(vector<ll> &f, bool invert) {
	int n = f.size();
	if(n == 1) return;
	...
	ntt<modulo>(f0, invert);
	ntt<modulo>(f1, invert);
	ll w = 1;
	ll wn = qpow<modulo>(primitive_root, (modulo - 1) / n);
	if(invert) {
		wn = qpow<modulo>(wn, modulo - 2);
	}
	for(int i = 0; i < n / 2; ++i) {
		ll u = f0[i];
		ll v = w * f1[i] % modulo;
...
template<ll modulo, ll primitive_root>
vector<ll> multiply(const vector<ll> &a, const vector<ll> &b) {
	...
	vector<ll> ca(n), cb(n);
	for(int i = 0; i < a.size(); ++i) {
		ca[i] = norm<modulo>(a[i]);
	}
	for(int i = 0; i < b.size(); ++i) {
		cb[i] = norm<modulo>(b[i]);
	}
	ntt<modulo>(ca, false);
	ntt<modulo>(cb, false);
	for(int i = 0; i < n; ++i) {
