...
TEST_CASE("seg_build_and_query") {
	vector<SumInfo> v;
	for(int i = 1; i <= 5; ++i) v.push_back(SumInfo{i});
	SegTree<SumInfo, SetApplier> seg(v);
	REQUIRE(seg.query(0, 5).val == 15);
	REQUIRE(seg.query(1, 4).val == 9);
	REQUIRE(seg.query(0, 1).val == 1);
	REQUIRE(seg.query(4, 5).val == 5);
}

TEST_CASE("seg_point_update") {
	vector<SumInfo> v;
	for(int i = 1; i <= 5; ++i) v.push_back(SumInfo{i});
	SegTree<SumInfo, SetApplier> seg(v);
	seg.update(2, SetApplier{10});
	REQUIRE(seg.query(0, 5).val == 22);
	REQUIRE(seg.query(2, 3).val == 10);
	REQUIRE(seg.query(1, 4).val == 16);
}

TEST_CASE("seg_empty_range") {
	vector<SumInfo> v;
	for(int i = 0; i < 8; ++i) v.push_back(SumInfo{1});
	SegTree<SumInfo, SetApplier> seg(v);
	REQUIRE(seg.query(3, 3).val == 0);
}

TEST_CASE("seg_assign_constructor") {
	SegTree<SumInfo, SetApplier> seg(4);
	REQUIRE(seg.query(0, 4).val == 0);
	seg.update(0, SetApplier{7});
	seg.update(3, SetApplier{3});
	REQUIRE(seg.query(0, 4).val == 10);
	REQUIRE(seg.query(1, 3).val == 0);
}
...
TEST_CASE("seg_max_query") {
	vector<MaxInfo> v;
	for(ll x : {3LL, 1LL, 4LL, 1LL, 5LL, 9LL, 2LL, 6LL})
		v.push_back(MaxInfo{x});
	SegTree<MaxInfo, MaxSetApplier> seg(v);
	REQUIRE(seg.query(0, 8).val == 9);
	REQUIRE(seg.query(0, 5).val == 5);
	REQUIRE(seg.query(5, 8).val == 9);
	seg.update(5, MaxSetApplier{0});
	REQUIRE(seg.query(0, 8).val == 6);
}
