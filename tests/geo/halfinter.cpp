...
TEST_CASE("halfinter_unit_square") {
	...
	vector<Line> lines = {
		Line(Point(0.0l, 0.0l), Point(1.0l, 0.0l)),
		Line(Point(1.0l, 0.0l), Point(0.0l, 1.0l)),
		Line(Point(1.0l, 1.0l), Point(-1.0l, 0.0l)),
		Line(Point(0.0l, 1.0l), Point(0.0l, -1.0l)),
	};
...
TEST_CASE("halfinter_right_triangle") {
	...
	// y >= 0: Line((0,0), dir(1,0))
	// x+y <= 2: Line((2,0), dir(-1,1)) → cross((-1,1),(x-2,y)) = -(x-2)-(-1)*y
	// = 2-x+y? Wait Let me verify: cross((-1,1),(x-2,y)) = (-1)*y - 1*(x-2) =
	// -y - x + 2 = 2-x-y >= 0 → x+y <= 2 ✓
	vector<Line> lines = {
		Line(Point(0.0l, 1.0l), Point(0.0l, -1.0l)),
		Line(Point(0.0l, 0.0l), Point(1.0l, 0.0l)),
		Line(Point(2.0l, 0.0l), Point(-1.0l, 1.0l)),
	};
	auto result = half_inter(lines);
	REQUIRE(result.size() >= 3);
	auto polygon = poly_from_lines(result);
...
TEST_CASE("halfinter_empty") {
	...
	// x >= 2: Line((2,0), dir(0,-1)) → cross((0,-1),(x-2,y)) = x-2 >= 0 → x >=
	// 2 These are contradictory (x <= 1 AND x >= 2)
	vector<Line> lines = {
		Line(Point(1.0l, 0.0l), Point(0.0l, 1.0l)),	 // x <= 1
		Line(Point(2.0l, 0.0l), Point(0.0l, -1.0l)), // x >= 2
	};
	auto result = half_inter(lines);
