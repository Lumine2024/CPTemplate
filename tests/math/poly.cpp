...
void ensure_poly_eq(const Poly &poly, const vector<Z> &expect) {
	ENSURE(poly.size() == expect.size());
	for(int i = 0; i < expect.size(); ++i) {
		ENSURE(poly[i] == expect[i]);
	}
}
